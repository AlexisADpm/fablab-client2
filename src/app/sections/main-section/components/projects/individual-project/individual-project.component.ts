...
export class IndividualProjectComponent {
  proyecto: any; // Puedes definir un tipo `project` si lo tienes
  id: number = 0;
  ...
  ngOnInit(): void {
    ...
    this.proyecto = this.dbLocalProyectos.find((p) => p.proyectId === this.id);
  }

  volver() {
    this.router.navigate(['/']); // Cambia por la ruta a tu onepage o donde quieras volver
  }
}
