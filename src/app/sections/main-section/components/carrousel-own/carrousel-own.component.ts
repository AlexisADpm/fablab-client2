...
export class CarrouselOwnComponent {
  ...
  ngAfterViewInit() {
    this.renderer.removeClass(
      this.navButttons.get(0)?.nativeElement,
      'text-gray-100/70'
    );
  ...
  moves(pos: number): void {
    //move +
    if (pos) {
      this.activePositionIndex.update((pos) =>
        pos < this.slides.length - 1 ? pos + 1 : 0
      );
    }
    //move -
    else {
      this.activePositionIndex.update((pos) =>
        pos > 0 ? pos - 1 : this.slides.length - 1
      );
    }
    this.renderer.setStyle(
      this.carrousel.nativeElement,
      'transform',
      `translateX(${-this.containerWidth() * this.activePositionIndex()}px)`
    );
  ...
  changeDotColor(pos: number): void {
    ...
    this.renderer.removeClass(
      this.navButttons.get(pos)?.nativeElement,
      'text-gray-100/70'
    );
    this.renderer.addClass(
      this.navButttons.get(pos)?.nativeElement,
